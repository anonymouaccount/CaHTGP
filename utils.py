...
class Data(Dataset):
    ...
    def __init__(self, data, max_n_neighbor=8, train_len=None):
    ...
    def __getitem__(self, index):
        ...
        # === SESSION SCALE: Local adjacency (4 edge types) — same as DMI-GNN ===
        ...
        # === FAST SCALE: Temporal neighbor sequences ===
        # Reconstruct original temporal order (u_input is reversed)
        original_seq = [u_input[i] for i in range(len(u_input)) if u_input[i] != 0]
        original_seq = list(reversed(original_seq))

        neighbor_seqs = np.zeros((max_n_node, self.max_n_neighbor), dtype=np.int64)
        neighbor_masks = np.zeros((max_n_node, self.max_n_neighbor), dtype=np.float32)

        for node_idx in range(n_unique):
            if node[node_idx] == 0:
                continue
            item_id = node[node_idx]
            neighbors = []
            for pos in range(len(original_seq) - 1):
                if original_seq[pos] == item_id:
                    neighbors.append(original_seq[pos + 1])
            if len(neighbors) > self.max_n_neighbor:
                neighbors = neighbors[-self.max_n_neighbor:]
            for j, nb in enumerate(neighbors):
                neighbor_seqs[node_idx][j] = nb
                neighbor_masks[node_idx][j] = 1.0
        ...
        return [torch.tensor(alias_inputs), torch.tensor(adj), torch.tensor(items),
                ...
                torch.tensor(neighbor_seqs), torch.tensor(neighbor_masks)]
